...
def create_pdf_report(patient_data: PatientData, risk_data: RiskData):
    pdf = PDFReport()
    pdf.add_page()
...
if baseline_risk:
    ...
    with st.form("treatment_form"):
        ...
        if submitted:
            ...
            if patient_name:
                patient = PatientData(name=patient_name, age=age, sex=sex)
                risk = RiskData(
                    baseline_risk=baseline_risk,
                    final_risk=final_risk,
                    current_ldl=ldl,
                    ldl_target=target_ldl,
                    recommendations=recommendations
                )
                pdf_bytes = create_pdf_report(patient, risk)
